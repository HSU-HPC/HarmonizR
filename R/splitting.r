...
splitting <- function(affiliation_list,
  ...
  integer_affil <- as.integer(factor(affiliation_list, levels = unique(affiliation_list)))
  ...
  ordered_main_data <- ordered_main_data[1:(length(ordered_main_data)-1)]
  ...
  # Print to console
  if (verbosity >= 4) {
    print(paste0("Dissection starting time (parallel): ", Sys.time()))
  }
  ...
  created_subdfs <- foreach::foreach(current_ind = indices_to_care_for, .combine = c) %dopar% {
    
    # This line should give us all indices
    row_indices_to_use <- which(sorted_integer_affil %in% sorted_integer_affil[current_ind])
    # These lines should give us all indices for the columns we need to use
    vec_in_affil <- unlist(affiliation_list[match(sorted_integer_affil[current_ind], integer_affil)])
  ...
  # Print to console
  if (verbosity >= 4) {
    print(paste0("Dissection finishing time (parallel): ", Sys.time()))
  }
  
  # Print to console
  if (verbosity >= 5) {
    print(paste0("Amount of sub-dataframes to-be-adjusted: ", length(created_subdfs) - 1))
    for (current_subdf in created_subdfs) {
      if (ncol(current_subdf) == 0) {
        print(paste0("Features within the empty sub-dataframe (discraded): ", nrow(current_subdf)))
      }
    }
  }
  ...
  # Print to console
  if (verbosity >= 4) {
    print(paste0("Adjustment starting time (parallel): ", Sys.time()))
  }
  ...
  cured_subdfs <- foreach::foreach(current_subdf = created_subdfs, .combine = c) %dopar% {
    
    description <- data.frame()
    
    # This if-statement is important, since the empty sub-dataframe would crash
    # the loop since no description can by build for a dataframe with 0 columns
    if (ncol(current_subdf) > 0) {
    ...
    if (length(unique(ComBat_input_batch)) == 1){
      ...
      # only a failsave. The second argument ensures trashing empty dataframes
      if (nrow(ComBat_input_dat) > 1 && length(unique(ComBat_input_batch)) > 1) {
  ...
  # Print to console
  if (verbosity >= 4) {
    print(paste0("Adjustment finishing time (parallel): ", Sys.time()))
  }
